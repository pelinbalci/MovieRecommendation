...
def show_train_predict_page_v1():
    ...
    st.write("""#### Enter your own ratings!""")
    ...
    st.write(len(all_genres_df_2))
    st.write(all_genres_df_2.head())
    ...
    if not selected_genres:
        ...
        print('CONCATTED: ')
        print(all_genres_df_3)

        all_genres_df_3 = all_genres_df_3.sample(len(all_genres_df_3), random_state=randomstate)
        print('FINAL ALLGENRES 3: ')
        print(all_genres_df_3)

    st.write(len(all_genres_df_3))
    st.write('FİNAL', all_genres_df_3.head())
